...
filter_missing_vals= function(vector_of_peptide){ #vector of peptide is a list of normalised peptide values 
  any(is.na(vector_of_peptide))
}
...
TMT= readxl::read_xlsx(
  "~/Documents/Rhys_Proteomics/Data/Rhys Morgan 120122 6Plex TMT_NB4 RM.xlsx")
...
log_frame_output= log_frame_output[!is.na(log_frame_output$logFC), ] #as there are values where the intensities are the same through out- messes up ggplot below
...
ggplot(data=log_frame_output, aes(x=logFC, y=-log10(P.Value),col=Expression,label=label)) +
  ...
  labs(y= "-log10(p value)",x="Fold change",title ="Volcano plot for β-catenin shRNA NB4 with top 20 upregulated and downregulated proteins highlighted") +
...
ggsave("~/sRNA_NB4_volcano.tiff",width=5000,height= 2000,units = "px")
...
tiff("~/heatmap_NB4_shRNA.tiff",width=1200,height= 800,units = "px")
heatmap.2(heatmap_matrix,main="NB4 shRNA sample heatmap with dendrogram",cexRow = 1,density.info="none",
