...
TMT= readxl::read_xlsx(
  "~/Documents/Rhys_Proteomics/Data/Rhys Morgan 191021 6Plex TMT RM.xlsx")
...
TMT[,15:20]= bcv::impute.svd(TMT[,21:26], 1 )$x 
################################################################################
#PCA graph
################################################################################
PCA_df= t(TMT_matrix)
...
ggplot(data=log_frame_output, aes(x=logFC, y=-log10(P.Value),col=Expression,label=label)) +
  ...
  labs(y= "-log10(p value)",x="Fold change",title ="Volcano plot for β-catenin CRISPR knockout with top 20 upregulated and downregulated proteins highlighted") +
...
ggsave("~/CRISPR_NB4_volcano.tiff",width=5000,height= 2000,units = "px")
...
tiff("~/NB4_CRISPR_heatmap.tiff",width=1200,height= 800,units = "px")
heatmap.2(heatmap_matrix,main="NB4 CRISPR Sample heatmap with dendrogram",cexRow = 1,density.info="none",
